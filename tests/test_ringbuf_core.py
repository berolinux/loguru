...
class TestPadding:
    def test_padding_transparent_to_reader(self, rb_path):
        ...
        # Write small frames to leave < 72 bytes remaining, then big frame
        writes = []
        for _ in range(4):  # 4*16=64 used, 64 remaining
            assert rb.write(small)
            writes.append(small)
        # 64 remaining < 72 needed → padding frame + big at offset 0
        # But first need room: drain to free space
        rb.drain()
        # Re-fill to trigger padding
        for _ in range(4):
            rb.write(small)
        assert rb.write(big)
