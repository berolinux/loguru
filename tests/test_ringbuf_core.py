...
class TestPadding:
    def test_padding_transparent_to_reader(self, rb_path):
        ...
        Setup (capacity 256 bytes):
          - Write 128 bytes (8 frames of 16).
          - Read 96 bytes (6 frames) to advance rpos and free front space.
          - Write 112 more bytes (7 frames) to position near end (wpos=240).
          - remaining at 240 = 16 < 48 (big frame) → padding + wrap write.
        """
        cap = 256
        rb = RingBuffer(rb_path, capacity=cap, create=True)
        tiny = b"t" * 7          # frame size = 16
        big = b"B" * 39           # frame size = align8(9+39) = 48

        # Fill 128 bytes (8 frames)
        for _ in range(8):
            assert rb.write(tiny)

        # Read 6 frames (96 bytes) to advance rpos
        for _ in range(6):
            assert rb.try_read() == tiny
        # Now: rpos=96, wpos=128, used=32, free=224

        # Write 7 more frames (112 bytes) → wpos=240
        for _ in range(7):
            assert rb.write(tiny)
        # State: wpos=240, rpos=96, used=144, free=112
        # remaining at 240 = 16 < 48 → will trigger padding
