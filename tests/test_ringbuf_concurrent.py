...
class TestConcurrentWriterAndReader:
    def test_live_drain(self, shm_path):
        ...
        # Due to timing and potential torn frames at process exit boundary,
        # we may not see every single frame the writer reported. Accept being
        # off by a tiny amount (torn/crashed last frame scenario), but ensure
        # we got the vast majority and all received frames are valid.
        assert len(collected) >= written - 10, f"lost too many frames: {len(collected)} vs {written}"
        # Verify all we did get are intact
